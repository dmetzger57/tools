  ...
  if (rc == SQLITE_ROW) {
    ...
    if (!verifyChecksum && mtime_match) {
      ...
      if (verifyChecksum && checksum_match) {
        ...
        const char *reason =
            (!mtime_match) ? "CHANGED (Metadata)" : "CHANGED (Checksum)";
        log_message(reason, path);

        if (update) {
          sqlite3_stmt *up_stmt;
          sqlite3_prepare_v2(db,
                             "UPDATE files SET checksum = ?, last_modified = ? "
                             "WHERE full_path = ?",
                             -1, &up_stmt, NULL);
    ...
    if (update) {
      ...
      sqlite3_prepare_v2(
          db,
          "INSERT INTO files (file_name, full_path, size, created, "
          "last_modified, owner, checksum) VALUES (?, ?, ?, ?, ?, ?, ?)",
          -1, &ins_stmt, NULL);
...
void traverse_directory(const char *dir_path) {
  ...
  while ((entry = readdir(dir))) {
    ...
    if (stat(full_path, &st) == 0) {
      ...
      else if (strstr(full_path, ".DS_Store") == NULL)
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 1; i < argc; i++) {
    if (strcmp(argv[i], "-p") == 0)
      ...
      fprintf(
          stderr,
          "Usage: [-c] [-u] [-v] [-l] [-t ThreadCount] -d DbName -p Path\n");
  ...
  snprintf(log_path, sizeof(log_path), "%s/%s-%s.log", log_dir, db_name,
           timestamp);
  ...
  sqlite3_exec(
      db,
      "CREATE TABLE IF NOT EXISTS files (id INTEGER PRIMARY KEY, file_name "
      "TEXT, full_path TEXT UNIQUE, size INTEGER, created INTEGER, "
      "last_modified INTEGER, owner TEXT, checksum TEXT, keywords TEXT);",
      0, 0, 0);
  sqlite3_exec(
      db,
      "CREATE TABLE IF NOT EXISTS meta (id INTEGER PRIMARY KEY AUTOINCREMENT, "
      "last_checksum_verify_date TEXT, last_date_verify TEXT, verify_machine "
      "TEXT, num_unchanged INTEGER, num_changed INTEGER, num_new INTEGER, "
      "num_missing INTEGER, num_errors INTEGER);",
      0, 0, 0);
  ...
  sqlite3_stmt *pStmt;
  sqlite3_prepare_v2(db,
                     "SELECT last_checksum_verify_date, last_date_verify, "
                     "num_unchanged, num_changed, num_new, num_missing, "
                     "num_errors FROM meta ORDER BY id DESC LIMIT 1",
                     -1, &pStmt, NULL);
  if (sqlite3_step(pStmt) == SQLITE_ROW) {
    ...
    pU = sqlite3_column_int(pStmt, 2);
    pC = sqlite3_column_int(pStmt, 3);
    pN = sqlite3_column_int(pStmt, 4);
    pM = sqlite3_column_int(pStmt, 5);
    pE = sqlite3_column_int(pStmt, 6);
  }
  ...
  char *sql;
  asprintf(&sql,
           "INSERT INTO meta (%s, verify_machine, num_unchanged, num_changed, "
           "num_new, num_missing, num_errors) VALUES "
           "(datetime('now','localtime'), ?, ?, ?, ?, ?, ?)",
           col);
  sqlite3_stmt *insMeta;
  sqlite3_prepare_v2(db, sql, -1, &insMeta, NULL);
  sqlite3_bind_text(insMeta, 1, hname, -1, SQLITE_STATIC);
  sqlite3_bind_int(insMeta, 2, unchangedCount);
  sqlite3_bind_int(insMeta, 3, changedCount);
  sqlite3_bind_int(insMeta, 4, newCount);
  sqlite3_bind_int(insMeta, 5, missingCount);
